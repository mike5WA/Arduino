...
Display fridge cavity temperature and bar graph of fan speed
Measures temperature in fridge cavity and adjusts extractor fan speeds accordingly
Temperature obtained from DHT22 on D2 & fan speed calculated dependant upon temperature
Digital 3 used as PWM connected to npn transistor taking value from fan speed
Transistor in turn provides PWM control of battery power to 2 12v fans
To dim display at night light sensor used to adjust TFT led brightness

Arduino Pins:
 A2  	Light sensor
 D2 	DHT22 temp & humidity sensor.
 D3 	PWM pin for fan	  
 D4   	N/C
 D5   	N/C
...
//const byte PWM_595 = 3;         //Was pin driving 74HC595
...
int fanPin = 3;         //pin for fan control via npn transistor
...
float dht_OffLine (float temp_C)
{
   if (isnan(temp_C))
   {
      do
      {
        analogWrite(fanPin, 255);   // Fan to full if sensor failing
